...
public class NewItemWork extends AppCompatActivity {
    ...
    private void savenote(){
        ...
        if(newimguri!=null){
                    ...
                    .addOnCompleteListener(task -> {
                        if (task.isSuccessful()){
                            FirebaseStorage.getInstance().getReference().child(imgname).getDownloadUrl().addOnSuccessListener(task2 -> {
                                ...
                                CollectionReference dbWorkRef = FirebaseFirestore.getInstance().collection("users");
                                dbWorkRef.document(a).collection("WorkRef").add(new Set_item(task2.toString(),title,"Qty available (kg):",initialquantity)).addOnSuccessListener(t2->{
            ...
            CollectionReference dbWorkRef = FirebaseFirestore.getInstance().collection("users");
            dbWorkRef.document(a).collection("WorkRef").add(new Set_item("",title,"Qty available (kg):",initialquantity));
            Toast.makeText(this, "Item added", Toast.LENGTH_SHORT).show();
            finish();
        }
    }
