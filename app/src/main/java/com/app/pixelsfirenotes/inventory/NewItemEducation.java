...
public class NewItemEducation extends AppCompatActivity {
    ...
    private void savenote(){
        ...
        if(newimguri!=null){
                    ...
                    .addOnCompleteListener(task -> {
                        if (task.isSuccessful()){
                            FirebaseStorage.getInstance().getReference().child(imgname).getDownloadUrl().addOnSuccessListener(task2 -> {
                                ...
                                CollectionReference dbEducationRef = FirebaseFirestore.getInstance().collection("users");
                                dbEducationRef.document(a).collection("EducationRef").add(new Set_item(task2.toString(),title,"Qty available (kg):",initialquantity)).addOnSuccessListener(t2->{
            ...
            CollectionReference dbEducationRef = FirebaseFirestore.getInstance().collection("users");
            dbEducationRef.document(a).collection("EducationRef").add(new Set_item("",title,"Qty available (kg):",initialquantity));
            Toast.makeText(this, "Item added", Toast.LENGTH_SHORT).show();
            finish();
        }
    }
