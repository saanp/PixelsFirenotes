...
public class House extends AppCompatActivity {
    ...
    private FirebaseFirestore dbHouse = FirebaseFirestore.getInstance();
    private CollectionReference HouseRef = dbHouse.collection("users");
    private FirebaseAuth fAuth;
    private EAdapter adapter;
    private FloatingActionButton savebutton;
    private String asd;

    private Button House_button;
    private String search;
    private EditText searchbar_House;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        savebutton = findViewById(R.id.add_item_House);
        savebutton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(House.this, NewItemHouse.class));
            }
        });

        buildrecyclerview();

        searchbar_House=findViewById(R.id.searchbar_House);
        House_button=findViewById(R.id.House_button);

        House_button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                search= searchbar_House.getText().toString();
                Query query = HouseRef.document(asd).collection("HouseRef").orderBy("title").startAt(search).endAt(search + "\uf8ff");
    ...
    private void buildrecyclerview(){
        ...
        Log.d("test","fetching House for "+asd);
        Query query = HouseRef.document(asd).collection("HouseRef");
        FirestoreRecyclerOptions<Set_item> options = new FirestoreRecyclerOptions.Builder<Set_item>().setQuery(query, Set_item.class).build();
        adapter = new EAdapter(options,this);
        RecyclerView recyclerView = findViewById(R.id.recviewHouse);
    ...
    private void shareitem(Set_item note) {
        ...
        String shareSub="House ITEM SHARING";
