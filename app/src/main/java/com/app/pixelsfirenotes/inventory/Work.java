...
public class Work extends AppCompatActivity {
    ...
    private FirebaseFirestore dbWork = FirebaseFirestore.getInstance();
    private CollectionReference WorkRef = dbWork.collection("users");
    ...
    private EditText searchbar_Work;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        savebutton = findViewById(R.id.add_item_Work);
        savebutton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(Work.this, NewItemWork.class));
            }
        });

        buildrecyclerview();

        searchbar_Work=findViewById(R.id.searchbar_Work);
        Work_button=findViewById(R.id.Work_button);

        Work_button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                search= searchbar_Work.getText().toString();
                Query query = WorkRef.document(asd).collection("WorkRef").orderBy("title").startAt(search).endAt(search + "\uf8ff");
    ...
    private void buildrecyclerview(){
        ...
        Log.d("test","fetching Work for "+asd);
        Query query = WorkRef.document(asd).collection("WorkRef");
        FirestoreRecyclerOptions<Set_item> options = new FirestoreRecyclerOptions.Builder<Set_item>().setQuery(query, Set_item.class).build();
        adapter = new EAdapter(options,this);
        RecyclerView recyclerView = findViewById(R.id.recviewWork);
